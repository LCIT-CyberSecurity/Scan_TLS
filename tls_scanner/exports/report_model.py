...
from ..models import CertificateRevocationResult, CertificateTrustResult, EncryptionPolicy, ScanJob
from ..pki import REVOCATION_NOT_TESTED, REVOCATION_REVOKED, TRUST_UNTRUSTED
...
def row_indexes(row: list[Any], crypto: str) -> dict[str, int]:
    if len(row) >= (27 if crypto == "pqc" else 26):
        ...
        return {
            ...
            "revocation_status": 21 + offset, "ocsp_status": 22 + offset, "crl_status": 23 + offset,
    ...
    if len(row) >= (24 if crypto == "pqc" else 23):
        ...
        return {
            ...
            "trust_classification": 17 + offset, "trusted_by": 18 + offset,
            "trust_anchor": 19 + offset, "chain_validation": 20 + offset,
            "revocation_status": 999, "ocsp_status": 999, "crl_status": 999,
            "compliance": -2, "reason": -1,
        }
    if len(row) >= 19:
        return {
    ...
    return {
        ...
        "trust_anchor": 999, "chain_validation": 999, "revocation_status": 999,
        "ocsp_status": 999, "crl_status": 999, "compliance": -2, "reason": -1,
    }
...
FINDING_DEFINITIONS = {
    ...
    "CERTIFICATE_REVOKED": (
        "Certificate Revoked", "PKI", "high",
        "The endpoint certificate was reported as revoked by an OCSP responder or CRL.",
        "Clients may reject the certificate, and continued use can indicate key compromise or improper certificate lifecycle handling.",
        "Remove the revoked certificate from service and deploy a valid replacement certificate.",
    ),
...
def classify_finding(reason: Any, tls_version: Any = "", cipher_suite: Any = "") -> str:
    ...
    if "certificate revoked" in normalized:
...
def fallback_trust_result(row: list[Any], indexes: dict[str, int]) -> CertificateTrustResult:
    ...
    trusted_by = tuple(item.strip() for item in trusted_by_value.split(",") if item.strip() and item.strip() not in {"None", "N/A"})
...
def certificate_trust_display_values(certificate):
    return ", ".join(certificate.trusted_by) or "N/A", certificate.trust_anchor or "N/A"


def fallback_revocation_result(row: list[Any], indexes: dict[str, int]) -> CertificateRevocationResult:
    return CertificateRevocationResult(
        revocation_status=str(row_value(row, indexes, "revocation_status", REVOCATION_NOT_TESTED) or REVOCATION_NOT_TESTED),
        ocsp_status=str(row_value(row, indexes, "ocsp_status", REVOCATION_NOT_TESTED) or REVOCATION_NOT_TESTED),
        crl_status=str(row_value(row, indexes, "crl_status", REVOCATION_NOT_TESTED) or REVOCATION_NOT_TESTED),
    )
...
def build_certificate(
    ...
    expires_within_days: int,
    trust_result: CertificateTrustResult | None = None,
    revocation_result: CertificateRevocationResult | None = None,
    ...
    revocation_result = revocation_result or fallback_revocation_result(row, indexes)
    status = "Valid"
    ...
    return CertificateObservation(
        ...
        ocsp_status=revocation_result.ocsp_status,
        crl_status=revocation_result.crl_status,
        status=status,
    )
...
def build_report_model(
    ...
    for endpoint_id, endpoint_rows in sorted(rows_by_endpoint.items()):
        ...
        revocation_result = job.certificate_revocation_results.get(endpoint_id) or fallback_revocation_result(first, indexes)
        certificate = build_certificate(first, indexes, job.certificate_expires_within_days, trust_result, revocation_result)
        cipher_suites = []
        endpoint_finding_ids: set[str] = set()
        untested_checks = ["Hostname validation", "SAN validation"]
        if certificate.chain_validation_status == "Not Tested":
            untested_checks.append("Trust chain validation")
        if certificate.revocation_status == REVOCATION_NOT_TESTED:
            untested_checks.append("Revocation status")
        if certificate.ocsp_status == REVOCATION_NOT_TESTED:
            untested_checks.append("OCSP status")
        if certificate.crl_status == REVOCATION_NOT_TESTED:
            untested_checks.append("CRL status")
        errors = []
        ...
        if certificate.revocation_status == REVOCATION_REVOKED:
            endpoint_finding_ids.add("CERTIFICATE_REVOKED")
            add_finding(grouped_findings, occurrences, "CERTIFICATE_REVOKED", endpoint_id, "; ".join(revocation_result.details) or "Certificate revoked", "KO", policy_ids, scan_timestamp)
        ...
        endpoint = EndpointReport(
            ...
            pki={
                ...
                "Trusted by": certificate_trust_display_values(certificate)[0],
                "Trust anchor": certificate_trust_display_values(certificate)[1],
                ...
                "Revocation status": certificate.revocation_status,
                "OCSP status": certificate.ocsp_status,
                "CRL status": certificate.crl_status,
            },
            pqc={
...
def build_metadata_document(model: ReportModel, basename: str, written_files: list[str]) -> dict[str, Any]:
    return {
        ...
        "endpoint_revocation": [
            {
                "endpoint_id": endpoint.endpoint_id,
                "revocation_status": endpoint.certificate.revocation_status,
                "ocsp_status": endpoint.certificate.ocsp_status,
                "crl_status": endpoint.certificate.crl_status,
            }
            for endpoint in model.endpoints
        ],
        "files": written_files,
