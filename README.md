...
PKI validation controls that are not currently executed are reported as `Not Tested`, including hostname validation, SAN validation, disabled trust-chain validation, and disabled revocation checks. `Not Tested` is not treated as compliant.
...
Limitations: the scanner still depends on Nmap output for observed TLS/certificate inventory data; hostname and SAN validation remain `Not Tested`; revocation checks require outbound access to certificate-declared OCSP or CRL HTTP(S) endpoints when enabled; the HTML charts are intentionally simple offline visuals rather than a heavy charting library.
...
intermediates through AIA, and it does not implement hostname validation or SAN
validation; those controls remain reported as not tested.

Certificate revocation checks are configured under
...
When enabled, TLS Scan checks OCSP responder URLs and CRL distribution points
declared by the leaf certificate, verifies signed responses against the issuer
certificate presented by the server, and marks revoked certificates as
non-compliant. Redirects are not followed, response size is bounded, and
private, loopback, link-local, multicast, or reserved revocation hosts are
blocked by default to reduce SSRF risk. Set `allow_private_urls: true` only for
controlled internal PKI scans where private OCSP or CRL endpoints are expected.
